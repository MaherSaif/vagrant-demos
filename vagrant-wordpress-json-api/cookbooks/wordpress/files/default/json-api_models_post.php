...
class JSON_API_Post {
  ...
  function save($values = null) {
    ...
    if (isset($values['custom_fields'])) {
     ...
     foreach ($values['custom_fields'] as $key => $value){

        update_metadata('post', $this->id, $key, (is_array($value)) ? json_encode($value) : $value);
     }
    ...
    if (isset($attachment_id)){
        $wp_values['post_content'] = wp_get_attachment_image($attachment_id, 'post-single').$wp_values['post_content'];
        $wp_values['ID'] = $this->id;
        wp_update_post($wp_values);
